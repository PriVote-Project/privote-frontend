...
export const VotingSection = ({
  ...
  const handleSelect = useCallback(
    (index: number) => {
      if (pollType === PollType.SINGLE_VOTE) {
        ...
        setIsVotesInvalid(Object.keys(isVotesInvalid).reduce((acc, key) => ({ ...acc, [key]: false }), {}));
      } else {
        setSelectedCandidate(index);
  ...
  const handleWeightedVoteChange = useCallback(
    (prevVotes: string | undefined, votes: string, index: number) => {
      ...
      if (maxVotePerPerson && currentTotalVotes - (Number(prevVotes) ?? 0) + Number(votes) > maxVotePerPerson) {
  ...
  return (
    <div className={styles['candidate-container']}>
      ...
      {votingState.message && pollStatus === PollStatus.OPEN && <p className={styles.message}>{votingState.message}</p>}
      {votingState.canVote && (
